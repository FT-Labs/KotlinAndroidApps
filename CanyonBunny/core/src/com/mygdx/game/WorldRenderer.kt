...
class WorldRenderer(val worldController: WorldController) : Disposable {
    ...
    private fun renderGuiExtraLives(batch: SpriteBatch) {
        ...
        if (worldController.lives>= 0 && worldController.livesVisual>worldController.lives){
            val i = worldController.lives
            val alphaColor = 0f.coerceAtLeast(worldController.livesVisual - worldController.lives - 0.5f)
            val alphaScale = 0.35f * (2 + worldController.lives - worldController.livesVisual) * 2f
            val alphaRotate = -45f * alphaColor
            batch.setColor(1f , 0.7f , 0.7f , alphaColor)
